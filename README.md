...
- **清洗版瘦身**：步骤 2 对 `2_clean_snapshot.html` 施加六规则（代码块→`code...` 占位、class/data-* 噪声剥除、astro 解包、空白压缩、隐藏子树折叠为 `data-u2m-hidden` 标记），隐藏判定走 juice 级联（沿用步骤 5 引号防御），统一折叠不删除——正文与带样式版零丢失，清洗版实测 −78%
