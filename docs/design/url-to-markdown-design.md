...
> 注：§6.1-6.4 与 §5 数据流描述的是早期脚本形态；现行管线（步骤 0-5、
> `snapshot.mjs` 单入口含清洗（2026-09-11 起原步骤 1/2 合并，清洗主体平移
> `lib/clean-snapshot.mjs`）等）的演进见
> `docs/superpowers/specs/` 系列设计文档与 CLAUDE.md。本节为原步骤 2（清洗）的增补记录。
