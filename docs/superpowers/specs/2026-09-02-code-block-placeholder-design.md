...
- 元素：`getComputedStyle(el).display` 匹配 `/^(inline|contents|ruby)/` 视为行内
  ...
  **空白守卫结构化（2026-09-03 修订，pre 2874/3127 实测）**：吞空白从
  「纯空白 + 当前行空」内容条件升级为结构条件——仅当两侧紧邻皆块级元素或
  容器边缘、且非行内元素的独子时零贡献；行内流空白（行首缩进 token span、
  行尾 `\n`、空行本体）按 pre 语义保留。旧条件在每次断行后必命中，吞掉
  缩进与空行（content_loss 空白不敏、rendered_mismatch 只比行数，均不可见）。
