...
彼处带保护集（titleIds 等 key 元素不拆）、此处 step 2 在 step 3 之前无保护集
——裸 span 是行内包装、内容流入可选块级父（p/section/h2-h6，实测占绝大多数），
无内容丢失。

**仅 clean 趟执行**：带样式版保留这些 span——其 `style` 携 `font-weight`/`color`
等供步骤 5 finalize 保留与步骤 7 LLM 判粗体/颜色，不能拆。由此孪生 id 集由
「相等」放宽为 `clean ⊆ styled`（step 3 在子集挑、step 4 在超集查恒命中）；
LONG_TEXT 占位符在共享段已赋值，拆包只挪位置不改 k，占位符集合守卫不受影响。
span 限定——div 等块级可能承 trans2img 模块边界，不碰。置于 K9 之后。

### 规则间顺序

K1 → K2 → K3 → K4 → K5 → K6 → K7 → K8 → K9 → K10。折叠类（K5/K6/K7）目标消失即
容忍跳过；K8 在全部折叠之后，只处理残余文本；K10 末段拆空壳 span。
...
- 2026-09-02（八次修订，2026-09-03 补记）：**K11 纯视图文本折叠（仅清洗版）**。动机：可视模块（图表轴刻度、图解步骤、对比卡片、KaTeX 视觉孪生 katex-html）内部的 div/span+文本碎片对步骤 3 是噪声（可视模块整棵标记、内部不拆），却占大量输入字节。机制照 K5 HIDDEN_TAG：极大纯子树整棵折为 `{{VIEW_TEXT|n_chars/n_words}}`、壳保留（data-idx 可引用、class/aria-label 标识模块身份）、无编号不进恢复清单——原文在带样式版，步骤 4-8 零影响。三步演进：初版（2026-09-02）极大纯 div/span 子树 + 壳保留；两道门槛（2026-09-03，防小内容误折）——文本量 ≥8 汉字/≥6 词（viewTextSize 逐文本节点求和，textContent 连接会把刻度行塌缩成 1 词）+ 结构量分档（纯 div 树内部 div>6、含 span 树 div/span 合计>4，兼保证折叠恒有字节收益）；p>span 形态 + 去 LT 限制（2026-09-03）——p 根独立一档（子树只许 text/span、p 不入纯树保正文段落流 `<div><p>…</p></div>` 不整块折叠）、含 LT 模块整棵折叠 LT 随折吞没（孪生守卫由「两版 LT 集合相等」放宽为 clean ⊆ styled）。豁免 a/button/h1-h3 后代（判读信号），hidden（K5 领地）/svg 阻断。
- 2026-09-03（九次修订）：**K11 行内允许集扩展 + 长文本占位移出共享段（clean 后置无编号）**。动机：①允许集仅 div/span 时模块内任一 strong/em/code/br/a 或行内公式（MathML）都阻断纯性——openai 参考页的行内 code 引用段、katex 公式段全部折不了；②clean 版 LT 编号与还原链无关（清洗版唯一消费者是步骤 3，只看结构+体量），却强迫两趟同位执行、编号逐一对应。改动两项：
  - **行内允许集扩展**：a/strong/b/em/i/code/br/MathML + K9 INLINE_TAGS 同族剔 img（u/s/mark/small/sub/sup/abbr/cite/q/kbd/samp/time）。math 整棵放行（内部 mi/mo/mn 不逐一检查——LaTeX 还原链走带样式版，步骤 6 才是 math 消费者）；img/svg 图片信号、块级/语义标签仍阻断。结构门槛计数同步扩为 div+行内全集（math/br 各计 1），分档不变（纯 div 树 >6 / 含行内树 >4）。
  - **LT 占位移出共享段**：styled 趟在分支开头带编号执行（`{{LONG_TEXT_k|n_unit}}`，2_long_text.json 由此产出——DOM 文本状态与共享段时代等价，产物逐字节不变，golden 守护）；clean 趟在 K11 之后无编号执行（`{{LONG_TEXT|n_unit}}`）——K11 先整棵折叠纯视图模块（viewTextSize 量的就是原文），幸存文本节点再占位；K6/K7 已先折 table/pre、无交叉。孪生守卫改为 clean LT 规模后缀 ⊆ styled。
  实测三参考页：styled 与 2_long_text.json 逐字节不变（sha256）；clean mmh1 prompt-cache 14213→12198B（−14.2%）、mmh1 ai-article-skill 22162→20913B（−5.6%）、openai prompt-caching 51364→40416B（−21.3%，行内 code/公式段解锁贡献最大）；viewText 计数 6→8 / 4→7 / 12→13；clean 零带编号 LT 残留、后缀守卫零违例。
  测试：新增行内集混排折叠、MathML 段落折叠两用例；既有 ~10 处 clean LT 断言改无编号形态（含守卫用例改「styled 带编号 / clean 无编号 + 后缀对应」）；K10/K11 孪生守卫改后缀比较；p>span 用例的 code 阻断控制组换 img（code 已入允许集）。golden 未变（styled 与 longtext 逐字节不变）。
