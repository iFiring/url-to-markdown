...
2. **尾块合并**：装箱后若最后一块段落块数 <5 且（前块 own + 尾块 own）≤
   `splitThreshold` → 并入前一块；守护不过则保留小块（5 个巨段落块的尾部
   可达 200KB+，无守护会合并出比不分割更糟的巨块）。合并后块数必然 ≥5，
   无需循环。
   - **N=1 回退**：合并后仅剩 1 块 → 视为未分割（不写 chunk 文件、走 §3.6
     不分割清理、emit `split=false`）——own ∈ (79KB, 80KB] 而文件略超 80KB
     的薄片边界会走到这里，等效于「触发判定被尾块下限否决」，接受；0.6% 级
     差异不影响可读性目标。
