...
| 2 | 块上限 | **主内容（own）向 50KB 靠齐**；📌开头/⚠️上文/⚠️下文上下文侧**不计入** 50KB 预算（2026-09-09 用户二次裁定：上下文是只读参照，为压 50KB 削上下文本末倒置）——每侧仍受自身选取上限约束（≤3/≤2/≤2 块且 ≤chunkMax/5），文件最坏 ≈ chunkMax + 3×cap。例外：单个段落块自身 >50KB 独立成块、允许溢出；尾块合并（#6）允许溢出至 80KB——贪心封边处恒有「前块 + 尾块首块 > 50KB」，50KB 守护会让尾块合并条件永假（死代码），故守护用 `splitThreshold`。（历史：v1 按文件字节超限削减上文→开头→下文（下文豁免预算），微信实测紧装中间包余量 102-464B 小于「1 块+注释」≈682B 的上下文最小空间、上下文被削光，且文档头尾开销使紧装块文件超 chunkMax；v2 装箱预留 3072B 仍被胖开头侧吃掉——两版均废弃） |
...
1. **贪心装箱**：按文档序累加段落块字节，加入下一块会超 `chunkMax` 即封块
   （own 向 50KB 靠齐）。单个段落块自身 > `chunkMax` 时独立成块（溢出例外）；
   该块与前后块的边界由贪心自然形成（前一块在它之前封块，它之后从下一块
   重新起块）。
...
计入本块文件），并**包裹在 HTML 注释内、每块独立一行**（2026-09-09 用户
裁定：注释对 DOM 解析不可见、对子代理的文本阅读可见——结构上杜绝误转换；
序列化保证文本节点中 `>` 转义为 `&gt;`，内容不会提前终结注释）：
...
- **副本变换（2026-09-09 用户裁定）**：上下文段落块剥 `data-idx`（只读参照
  无需选择器锚点，也强化「勿转换」信号——待转换块才带编号）与 `style`
  （内联样式对只读参照是纯字节噪音；其余属性保留），并压缩块内标签间空白与文本节点
  两缘空白（含 `&nbsp;` 实体；块与块之间的独立换行保留）。序列化保证
  文本中 `<`/`>` 已转义，变换正则只触达标签语法；own 区保真不压缩。
- **上下文不计入块预算（2026-09-09 用户裁定）**：三侧按选取规则就位后不再
  削减——50KB 预算只约束 own 主内容，上下文是只读参照、超出无妨；每侧自身
  选取上限（≤3/≤2/≤2 块且 ≤chunkMax/5）即全部约束，文件最坏 ≈
  chunkMax + 3×cap。（历史：v1 曾设「超限削减」——按（文件 − 下文侧）>
  chunkMax 触发、上文→开头→下文整侧清空（下文豁免预算），v2 改装箱预留
  3072B；微信实测两版都把紧装中间块的上下文削光/挤光，最终裁定上下文完全
  免费，削减机制整体废除）
...
第 X 块（2 ≤ X ≤ N−1）完整结构（标签一句话中英双语、文案实现照抄；上下文
段落块在注释内**每块独立一行**——2026-09-09 简化：长说明移交指南，标签只留
身份）：

```html
<body style="…">
<!-- 📌 开头上下文（勿转换）/ OPENING CONTEXT (do NOT convert)
[开头段落块]
[开头段落块…]
-->
<!-- ⚠️ 上文上下文（勿转换）/ PRECEDING CONTEXT (do NOT convert)
[上文段落块]
[上文段落块…]
-->
<!-- ✅ 待转换内容自此开始 / Convert ONLY the content below this marker -->
[本块自己的段落块]
<!-- ❌ 待转换内容自此结束 / Convertible content ENDS here -->
<!-- ⚠️ 下文上下文（勿转换）/ FOLLOWING CONTEXT (do NOT convert)
[下文段落块]
[下文段落块…]
-->
</body>
...
- ✅/❌ **每块恒在**（2026-09-09 修订，见 §9）：own 恒被一对 ✅/❌ 围绕——
  第 1 块 ✅ 紧跟 body 开标签（无 📌/⚠️上文 时它是 body 第一个节点）；
  最后一块 ❌ 紧贴 `</body></html>`（无 ⚠️下文 时它是 body 最后一个节点）
- 第 1 块：无 📌/⚠️上文；尾部有 ⚠️下文（有下文时）
- 最后一块：无 ⚠️下文
- 注释省略规则（仅三侧上下文）：某侧帽取后为 0 块 → 该侧注释整体省略
- own 区段落块逐字节拼接（与 `6_article.html` body 内容一致），不插换行
...
- **纯函数单测**（`lib/chunk-article.mjs`，不起浏览器）：
  ...
  - 贪心装箱 own ≤ chunkMax（上下文不计预算、无削减）；单个巨块独立成块且溢出豁免
  - 尾块合并：<5 块并入；80KB 守护不过则保留
  - 上下文：三侧数量/字节帽/去重（上下文不计预算——中间块五标记齐备、
    首块仅下文侧、末块无下文侧）
  - 标记注释：✅/❌ 每块恒在（首块 ✅ 紧跟 body 开标签、末块 ❌ 紧贴
    `</body></html>` 的位置断言）、三侧注释省略规则、K 动态数量、中英文案
  - 第 1 块与最后一块的特殊结构
...
- 2026-09-09（实施后修订）：**✅/❌ 转换边界标记每块恒在**。动机：原省略
  规则下首块无 ✅（无前侧上下文）、末块无 ❌（无下文侧）——首末块的转换
  边界依赖「无标记时转换全部」的隐含约定，对子代理文本阅读不直观。改动：
  组装循环无条件输出 START_MARK/END_MARK（✅ 恒在 own 前、❌ 恒在 own 后；
  三侧上下文注释照旧条件输出），首块 ✅ 紧跟 body 开标签、末块 ❌ 紧贴
  `</body></html>`；标记与上下文同款不计入块预算。指南「只转换 ✅ 与 ❌
  之间（无任何标记时转换全部）」判定不变——无标记形态只剩未分割的
  `6_article.html`。单测 T2-T7 断言同步反转 + 新增 T8 位置断言（✅/❌ 每块
  恰出现一次、首末位置紧贴）。
