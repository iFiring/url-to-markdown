...
| 2 | 块上限 | 每块文件 ≤ 50KB；唯一例外：单个段落块自身 >50KB 时独立成块、允许溢出 |
...
2. **尾块合并**：装箱后若最后一块段落块数 <5 且（前块 own + 尾块 own）≤
   `splitThreshold` → 并并前一块；守护不过则保留小块（5 个巨段落块的尾部
   可达 200KB+，无守护会合并出比不分割更糟的巨块）。合并后块数必然 ≥5，
   无需循环。
   - 已知边界：own 合计 ≤80KB 而文件总字节略超 80KB 的极端情形，合并回单块
     等效于「触发判定被尾块下限否决」——接受，0.6% 级差异不影响可读性目标。
...
| ⚠️ 上文 | 上一块尾部向内 | 2 | chunkMax/5 |
| ⚠️ 下文 | 下一块开头向外（最后一块无此侧） | 2 | chunkMax/5 |
...
- **超限削减**：配置后若本块文件总字节（头 + 注释 + 上下文 + own）>
  `chunkMax`，按 **下文整侧 → 上文整侧 → 开头整侧** 顺序清空，直到 ≤
  `chunkMax` 或上下文全空（own 自身超限的巨块例外不触发削减逻辑外的特殊路径
  ——同一条规则自然删光上下文，文件 = own + 头，保持「唯一例外」不变量）。
...
- **「恰引用一次」作用域**改写为「**本文件待转换部分**」
- **标题定级节**补一句：上文/开头上下文可用于建立标题层级与字号基准
