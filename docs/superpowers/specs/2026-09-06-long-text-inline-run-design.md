...
- 状态：已与用户确认设计（分节呈现逐节批准）；2026-09-07 自审修订：
  ...
  语义、void 元素序列化形态；2026-09-11 修订（§11）：强调归一 R1-R4 +
  定界符择优——§5.4 退化兜底收窄为防御路径；2026-09-11 修订（§12）：
  行内流空白折叠——源码排版空白不再拆行
...
- **强调边界退化**：`**…**` 内容首/尾为空白或 `*`/`_` 时 GFM 强调不闭合 →
  ...
  （2026-09-11 修订：见 §11——归一 + 定界符择优后该退化仅剩防御路径）
...
## 11. 2026-09-11 修订：强调归一与定界符择优（§5.4 退化兜底收窄为防御路径）

实测发现（微信长文 `mp.weixin.qq.com`）：公众号编辑器产出**双层加粗**——
`<strong>` 套 `font-weight:bold` span，§3.3 canonical 归一后成 strong 直接嵌
strong。外层强调内容首/尾触内层 md 记号（`*`）→ §5.4 退化兜底被批量触发：
单页 `9_markdown.md` 出现 117 处字面 `<strong>` 标签。且 `**a****b**` 的四星
连串触发 CommonMark rule of 3 拒绝拆分，**渲染出字面 `**`**——退化形态不只
源码丑，是保真损伤。

**修订**（消费端 `lib/inline2md.mjs` 单点，canonical 序列化零改动；既有
`2_long_text.json` 无需重跑步骤 2，重放步骤 8/9 即生效）：

1. **强调归一 R1-R4**（转换前 jsdom 预处理，迭代到不动点）：
   - R1 内容仅空白/br 的强调壳解包（子节点原位保留；run 中部 br 仍是硬换行）；
   - R2 同族（md 记号相同：strong/b、em/i、del/s）嵌套强调并为一层——仅当
     亲链全程同族才并，`<strong><em><strong>` 跨族不误并；
   - R3 强调元素首/尾的空白文本与 `<br>` 提升到元素外（GFM 强调空白边界
     本就不闭合，提升后渲染等价）；
   - R4 run 末尾悬空 `<br>` 剥离（穿透 a/行内透传族下降；条目以 `\` 收尾
     会被部分解析器渲染为字面反斜杠）。
2. **定界符择优**：内容首/尾触主记号字符（跨族嵌套产物，如 strong 包 em 的
   `*x*`）时换同语义备选定界符（`**`↔`__`、`*`↔`_`）——不同记号字符无
   rule of 3 冲突，marked --gfm 实渲等价（含三重嵌套 `**_**x**_**`）。
3. §5.4「强调边界退化 → raw HTML 透传」保留，但归一后空白边界不可达、
   `*` 边界被备选定界符接管——退化为纯防御路径。

**验证**：4 个真实页面全部 599 条 runs——raw 标签 118→0；487 条无病灶输出
字节级不变（变换单调：只在今天必然退化的结构上生效）；全部 run 文本签名
前后比对零丢失；微信页步骤 8/9 干净重放后 `9_markdown.md` 字面强调标签 0、
图片引用全部存在；全量单测 386/386 绿（`inline2md.test.mjs` 三条钉死退化
行为的断言翻转为新期望 + 新增归一/择优/尾部 br 用例）。

## 12. 2026-09-11 修订：行内流空白折叠（§5.3 转义策略补全）

实测发现（微信 description 块，`9_markdown.md` 第 3-5 行）：pretty-printed
页面源码在行内元素间夹排版空白（`\n + 14 空格缩进`），canonical 序列化原样
保留文本节点、escapeText 照抄不折叠——markdown 值被拆成多行（换行 + 缩进 +
尾部空白行）。浏览器按 `white-space:normal` 渲染时这些折叠为单空格，页面
实际是一行。

**修订**：escapeText 入口把文本节点空白 run（含换行）折叠为单空格
（`text.replace(/[ \t\r\n]+/g, ' ')`）——与 §5.2 code span 内部换行折叠、
math 源换行折叠同 rationale（浏览器行内流空白语义）：

- runs 是纯行内内容（pre 子树被 run 检测排除），文本节点换行从不产生可见
  换行；可见换行只来自 `<br>`（BR 分支不经 escapeText），硬换行不受影响；
- **不 trim run 首尾**：占位符与骨架短文本拼接（`前文{{LONG_TEXT_k}}后文`）
  时空白承载词间距，trim 会粘词；残余双空格 markdown 渲染时自折叠，无害；
- 行首中断符转义保留，lineStart 此后仅由值首与 `<br>` 触发——文本换行后
  的转义用例（`a\n- b`）期望翻转为 `a - b`（折叠后非行首，与浏览器渲染
  一致）；值首空白后的中断符仍转义（折叠后 ≤3 前导空格仍在 CommonMark
  中断窗内）；
- **texts 段不动**：其消费端含表格/代码内部展开（expandLongText），代码
  空白有语义不可折；实测 texts 0 换行，出现污染再立项。

**验证**：微信页 383 条 runs 仅 run 1（description）含可折叠空白——其余
382 条输出不变；重放步骤 8/9 后 description 多行合并为单行（残余双空格为
不 trim 的预期拼接产物）；全量单测 387/387 绿。
