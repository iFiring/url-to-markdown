...
5. `xl:hidden` 响应式块 → **不折叠（实现边界，钉行为测试固定）**：`@media` 规则 juice
   不内联（`removeStyleTags:true` 下含媒体查询的 `<style>` 原样保留、其声明从不写进
   style 属性），`__u2mDetectHidden` 读不到 `display:none`，响应式隐藏子树一律按可见
   保留（安全方向）；测试断言该内容**不折叠**、清洗版保留。
...
| @media 规则 juice 不内联 → 响应式隐藏（`xl:hidden`/`lg:hidden`）漏折 | 方向安全（多保留不误删）；实际影响：真实验收页仅 1 处 lg:hidden，近零；钉行为测试已固定该边界（见 §6 用例 5） |
