...
这是**一篇文章页面**， 读取 `<url-working-path>/2_clean_snapshot.html` 的 DOM 结构（元素层级、标签类型、语义 class）、长文本占位符（`{{LONG_TEXT_k|...}}`）与**文本规模 token** 分布，找到以下四类关键元素与列表流噪音的 `data-idx` (ID)：
...
## 约束
  ...
  3. **`hidden` 属性元素 + `{{HIDDEN_TAG|n_chars;n_a/n_div/...}}` 内容 token**：折叠的隐藏子树（模态/抽屉/移动端导航等）。根元素的 `data-idx` 可正常引用——原文完整保留在带样式版，纳入 listFlowIds 即可还原全文（FAQ 折叠答案、tab 变体面板是典型可纳入场景）；token 值是真实文本规模与标签构成（计数降序），可据此判断是否值得纳入
  ...
  6. **长文本占位符** `{{LONG_TEXT_k|n_chars}}` / `{{LONG_TEXT_k|n_words}}`：超阈值（>16 汉字 / >12 词）的**单个文本节点**折叠为编号占位符，k 与 `2_long_text.json`（及带样式版）一一对应、原文完整可还原；短文本（≤16 汉字 / ≤12 词）保留原文。占位符分布是判读线索——段落/标题/按钮的位置与体量看得到，`<title>` 原文保留（不占位）。**行内结构保真**：`<a>`/`<code>`/`<strong>` 等行内元素与混排短文本原样保留，不被折叠吞噬
  7. 链接与图片元素**不带 URL**（href/src 已清空，链接文本与 alt 保留）

  占位符形态速览：

  ```html
  <!-- 普通表格：2 行 × 2 列 -->
  <table data-idx="35">{{TABLE_TAG|2_rows|2_cols}}</table>
  <!-- 跨列表格：首行 1 个 colspan=3 的单元格 + 1 个尾列 = 4 列，取各行最大 -->
  <table data-idx="36">{{TABLE_TAG|3_rows|4_cols}}</table>
  <!-- 代码块：行数按换行切分（高亮 span 是语法 token、不是行）；
       语言线索在 data-language 属性 -->
  <pre data-idx="37" data-language="tsx">{{PRE_CODE_TAG|87_lines}}</pre>
  <!-- 纯代码块（无高亮、无语言） -->
  <pre data-idx="38">{{PRE_CODE_TAG|12_lines}}</pre>
  <!-- 长文本（中文按字符数 / 西文按词数），编号 k 与 2_long_text.json 对应 -->
  <p data-idx="39">{{LONG_TEXT_7|246_chars}}</p>
  <!-- 隐藏子树：规模 + 标签构成（计数降序） -->
  <div hidden data-idx="40">{{HIDDEN_TAG|120_chars;3_p}}</div>
  ```


## `2_clean_snapshot.html`结构示例：

> `data-idx` 的生成是在 DOM 树中自上而下自增的，所以通常只有两个情况：titleIds 或 descriptionIds 在 listFlowIds 里面，值更大，输出 JSON 里不应该有值；titleIds 或 descriptionIds 在 listFlowIds 前面，值更小。
...
<body>
  <div data-idx="1" class="xxx">
    <h1 data-idx="2">
      <!-- 全局唯一标题 titleIds[3] -->
      <span data-idx="3">Title...</span>
    </h1>
  </div>
  <div data-idx="4" class="xxx">

    <!-- 列表流 listFlowIds[5]（最外层文章流） -->
    <section data-idx="5" class="article">
      <!-- 元素 [6] 在 listFlowIds[5] 之内，不纳入 descriptionIds -->
      <div data-idx="6">
        <h2 data-idx="7">Author:</h2>
        <span data-idx="8">Name...</span>
      </div>
      <!-- 列表流噪音——元素 [9] 在列表流之内 → 必须纳入 listFlowDeleteIds -->
      <div data-idx="9" class="ad">Ad...</div>
      <p data-idx="10">
        <h2 data-idx="11">...</h2>
        <code data-idx="12">...</code>
      </p>
      <!-- 嵌套列表流 listFlowIds[14]：直接子元素含 <p> → 也是流，与 [5] 同标；
           步骤 6 最外层优先，[14] 整块作为 [5] 的子块迁入，[15]/[16] 不拆出 -->
      <div data-idx="14" class="sub">
        <p data-idx="15">子小节段落...</p>
        <p data-idx="16">子小节段落...</p>
        <!-- 噪音不限深度：嵌在嵌套流内部 → 同样纳入 listFlowDeleteIds -->
        <div class="ad" data-idx="17">内嵌广告...</div>
      </div>
      <!-- 非流：直接子元素全是行内 span/a，无段落级块 → 不标，
           整体作为 [5] 的子块迁入 -->
      <div class="tags" data-idx="18">
        <span data-idx="19">#tag1</span>
        <a data-idx="20">#tag2</a>
      </div>
      <!-- 代码块包装：装饰条 [22] + <pre>[23]，含段落级块 <pre> → 按判据
           也是流（标不标均可、多标无害——随外层 [5] 整块迁入，[22]/[23] 不拆出） -->
      <div class="codeblock" data-idx="21">
        <div class="bar" data-idx="22">tsx</div>
        <pre data-idx="23" data-language="tsx">{{PRE_CODE_TAG|23_lines}}</pre>
      </div>
      <!-- 非流：直接子元素仅 figure 复合单元 → 不标，整体迁入 -->
      <div data-idx="24"><figure data-idx="25">...</figure></div>
      <!-- hidden 折叠子树（{{HIDDEN_TAG|n_chars;构成}} token）：在流内无需
           特殊处理——原文在带样式版完整保留，整块迁入即还原全文（token 是
           真实文本规模与标签构成） -->
      <div hidden data-idx="26">{{HIDDEN_TAG|120_chars;3_p}}</div>
    </section>
    <!-- 列表流之外的广告无需标记——不在任何 key 元素子树内，步骤 4 自然裁掉 -->
    <!-- 列表流 listFlowIds[28]（与 [5] 互不嵌套的另一流） -->
    <div data-idx="28" class="article">
      <!-- 节头：序号 span + <h2>，含段落级块 <h2> → 按判据也是流，同标 -->
      <header class="head" data-idx="29">
        <span data-idx="30">01</span>
        <h2 data-idx="31">章节标题</h2>
      </header>
      <!-- 非流：figcaption 与表格包装都是复合单元、无段落级块 → 不标，整体迁入 -->
      <figure class="table" data-idx="32">
        <figcaption data-idx="33">表题</figcaption>
        <div data-idx="34"><table data-idx="35">{{TABLE_TAG|8_rows|4_cols}}</table></div>
      </figure>
      <p data-idx="36"><span data-idx="37">...</span></p>
      <!-- <dl> 本身是段落级块：作为 [28] 的一个子块整体迁入 -->
      <dl data-idx="38">
        <dt data-idx="39">术语</dt>
        <dd data-idx="40">释义</dd>
      </dl>
    </div>
    <!-- 手风琴项（不在任何外层流内）：直接子元素 = 展开/收起标题 button [43]
         （长问题文本折叠为 {{LONG_TEXT_k|n_chars}} 占位）+ 折叠内容流 [44]
         （hidden 折叠为 {{HIDDEN_TAG|200_chars;1_p}}——构成 1_p 即原
         <p> 子元素，含段落级块 → 是流）——按「含子流」判据项容器 [42] 也是
         流，标 [42] 与 [44] → button 作为子块整体迁入，标题文本保留 -->
    <div class="accordion-item" data-idx="42">
      <button data-idx="43">{{LONG_TEXT_3|80_chars}}</button>
      <div hidden data-idx="44">{{HIDDEN_TAG|200_chars;1_p}}</div>
    </div>
    <!-- 游离内容：[28]/[42] 的兄弟元素、不在任何流子树内（此处不把包装
         div 标成流——标它会把上方的流外广告一并带入）→ 整体单独标
         standaloneIds [46][47]，步骤 6 按文档序各成一块迁入 -->
    <h2 data-idx="46">小结</h2>
    <p data-idx="47">全文完。</p>
    ...
    <nav class="toc" data-idx="48">
      <ul data-idx="49">
        <li data-idx="50">01 为什么缓存主宰 Agent</li>
        <li data-idx="51">02 把提示词按缓存的方式来排版</li>
      </ul>
    </nav>
  </div>
</body>
