...
这是**一篇文章页面**， 读取 `<url-working-path>/2_clean_snapshot.html` 的 DOM 结构（元素层级、标签类型、语义 class）、长文本占位符（`{{LONG_TEXT_k|...}}`）与**文本规模 token** 分布，找到以下四类关键元素与列表流噪音的 `data-u2m-id` (ID)：
...
## 约束
  ...
  3. **`hidden` 属性元素 + `{{HIDDEN_TAG|n_chars;n_a/n_div/...}}` 内容 token**：折叠的隐藏子树（模态/抽屉/移动端导航等）。根元素的 `data-u2m-id` 可正常引用——原文完整保留在带样式版，纳入 listFlowIds 即可还原全文（FAQ 折叠答案、tab 变体面板是典型可纳入场景）；token 值是真实文本规模与标签构成（计数降序），可据此判断是否值得纳入
  ...
  <p data-u2m-id="39">{{LONG_TEXT_7|246_chars}}</p>
  <!-- 隐藏子树：规模 + 标签构成（计数降序） -->
  <div hidden data-u2m-id="40">{{HIDDEN_TAG|120_chars;3_p}}</div>
  ```
  ...
  <div data-u2m-id="4" class="xxx">
    ...
    <section data-u2m-id="5" class="article">
      ...
      <!-- hidden 折叠子树（{{HIDDEN_TAG|n_chars;构成}} token）：在流内无需
           ...
           真实文本规模与标签构成） -->
      <div hidden data-u2m-id="26">{{HIDDEN_TAG|120_chars;3_p}}</div>
    </section>
    ...
    <div class="accordion-item" data-u2m-id="42">
      <button data-u2m-id="43">{{LONG_TEXT_3|80_chars}}</button>
      <div hidden data-u2m-id="44">{{HIDDEN_TAG|200_chars;1_p}}</div>
    </div>
