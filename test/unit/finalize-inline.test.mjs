...
// =====CSS 关键字零值（initial/unset）=====
// 破坏点：关键字规则缺失 → 白名单内非继承属性上的 initial/unset 字面量存活

test('非继承属性上 initial/unset 全删（outline/align/flex/justify/overflow）', () => {
...
test('继承属性 unset 删（≡inherit 默认行为）、initial 保留（阻断继承有意义）', () => {
...
// 收紧后 display 走值门控（仅 flex/grid 四值存活），initial/unset 一律删
test('display:initial/unset 删（值门控吸收，不再保留块级标签上的行内化信号）', () => {
...
// =====transform:none + background 长手簇 =====
// 破坏点：无图规则缺失 → 非初始值 no-repeat 存活；clip 混入无图规则 → padding-box 误删

test('transform:none 删、真实 transform 保留', () => {
...
test('无图 background 长手簇全删（含非初始 no-repeat/left top）、background-color 保留', () => {
...
test('background-clip 仅删初始 border-box（clip 影响纯色绘制）', () => {
...
// =====UA 默认 display:inline =====
// 破坏点：无标签白名单 → span 上 display:inline 残留为噪音。
// （收紧后 div 上 inline / span 上 inline-block 也删——display
// 仅 flex/grid 四值存活，行内标签全值删，旧「保留真信号」语义已被取代）

test('行内默认标签 display:inline 删、其余声明保留', () => {
...
test('div display:inline 与 span display:inline-block 均删（值门控+行内全删）', () => {
...
test('与继承有效值等值的 font 声明删（normal≡400、根默认 medium≡16px）', () => {
...
test('对比保留——bold 下 400 重置、17px 下 18px', () => {
...
test('img height:auto 删、真实像素宽高保留', () => {
...
test('display 值门控——flex/inline-flex/grid/inline-grid 保留，其余值删', () => {
...
test('行内标签 display 全值删（inline-block/flex/grid 也不留）', () => {
...
test('五方向属性保留（flex-direction/wrap、grid-auto-flow/template-columns/rows）', () => {
...
test('对齐全族删——center 与 space-between/flex-start 同删', () => {
...
test('grid placement 删——容器方向信号（display:grid + template-columns/rows）保留', () => {
...
test('overflow 三件全删（auto/hidden/scroll/x/y——滚动裁剪不再保留）', () => {
