...
// 链路：步骤 4 render_article（真实 chromium）→ 合成步骤 5 分片 →
// 步骤 6（无 img/trans 纯 Node 路径，死端口 URL，合并 + 渲染 markdown）。
...
const URL = 'http://127.0.0.1:9/chunk-chain'; // 死端口：步骤 6 无 img/trans 不触网

test('分块链路：步骤 4 分割 → 分片骨架 → 步骤 6 合并 + 渲染 markdown', async () => {
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'u2m-chunk-chain-'));
  const urlDir = path.join(tmpRoot, urlToDirName(URL));
  fs.mkdirSync(urlDir, { recursive: true });
  fs.writeFileSync(path.join(urlDir, '2_clean_style_snapshot.html'), JUICED);
  ...
  // 合成步骤 5：每分片一个骨架分片（无占位符、无 img/trans）
  for (let x = 1; x <= N; x++) {
    fs.writeFileSync(path.join(urlDir, `5_skeleton_chunk_${x}_of_${N}.json`),
  ...
  for (const pos of order) assert.ok(pos >= 0, '6_markdown 应含全部分片内容');
