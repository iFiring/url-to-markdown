...
// 链路：步骤 3 render_article（真实 chromium）→ 合成步骤 5 分片 →
// 步骤 5（无 img/trans 纯 Node 路径，死端口 URL，合并 + 渲染 markdown）。
...
const URL = 'http://127.0.0.1:9/chunk-chain'; // 死端口：步骤 5 无 img/trans 不触网

test('分块链路：步骤 3 分割 → 分片骨架 → 步骤 5 合并 + 渲染 markdown', async () => {
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'u2m-chunk-chain-'));
  const urlDir = path.join(tmpRoot, urlToDirName(URL));
  fs.mkdirSync(urlDir, { recursive: true });
  fs.writeFileSync(path.join(urlDir, '1_clean_style_snapshot.html'), JUICED);
  fs.writeFileSync(path.join(urlDir, '2_key_ids.json'), JSON.stringify(KEY_IDS));
  fs.writeFileSync(path.join(urlDir, '1_snapshot.html'), JUICED);
  fs.writeFileSync(path.join(urlDir, '1_long_text.json'), JSON.stringify({ texts: {}, runs: {} }));
  const env = { U2M_WORKING_ROOT: tmpRoot, U2M_ARTICLE_SPLIT_THRESHOLD: '60000' };

  // 步骤 3：分割
  const r6 = await runScript(process.execPath, [path.resolve('script/render_article.mjs'), '--url', URL], { env, timeoutMs: 60000 });
  ...
  // 合成步骤 4：每分片一个骨架分片（无占位符、无 img/trans）
  for (let x = 1; x <= N; x++) {
    fs.writeFileSync(path.join(urlDir, `4_skeleton_chunk_${x}_of_${N}.json`),
  ...
  for (const pos of order) assert.ok(pos >= 0, '5_markdown 应含全部分片内容');
