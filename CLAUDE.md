...
# 调试行（阶段耗时、输入输出字节数、登录检测七信号命中、滚动轮次、逐图下载、
...
- **步骤 5 `compute_styles.mjs` —— 样式内联**
  ...
  - **零值过滤**（白名单内再过一场）——值等于全元素初始值的声明删除（边框按"边"语义：style none/缺省或 width 0 → 该边三件全删；box-shadow:none、background:transparent、radius:0px、overflow:visible 等精确值；font-size/weight 相对对比与 flex 布局信号不是零值、保留）——唯一元素级例外：`<img>` 的 width/height 保留（步骤 7 LLM 判图片权重——小图标/大图/图片组的信号）
...
**登录流程**：`snapshot-login.mjs` 对七个信号计分（全 frames 密码框 / URL 特征 / 标题与正文关键词——标题关键词只匹配 `<title>`、正文关键词只匹配正文 / 认证 cookie 反查 / 重定向 / SPA 等待 / **登录/注册按钮**——主 frame 元素级精确匹配：交互元素 a/button/[role=button] 看子树文本，其余（Vue/React 站常用 span/div 实现入口，实测极客时间即 cursor:pointer 的 span）看自身直接文本且须指针光标；文本 ≤8 字符、含「登录/登入/登陆/注册/log in/sign in/login/register/sign-in/log-in」、不含「退出/登出/logout/切换」、checkVisibility 可见；检测时刻一次 + 未定论时在 spa 等待窗内 400ms 轮询——SPA 迟水合（networkidle 提前达成、检测跑在头部渲染前）会让一次性检查随机扑空；不穿透 shadow DOM、不读 aria-label 是已知边界）；**两级制判定——`password`/`loginButton` 为强信号单独成立，其余信号 ≥2 命中成立**。**强信号裁决记忆**：viewer 工具栏的「✅ 登录完成」（recheck 通过时记 `login`）与「⏭️ 跳过登录」（记 `skip`，同时也刷新 storageState——用户可能已实际登录只是 recheck 永败，跳过不丢会话）兼作用户确认，按 `(hostname, 信号)` 写入 `working/cookies/login_decisions.json`（仅 snapshot-login 写，read-merge-write，损坏内容形状强转容忍）；`skip` 的信号在该域名**视为不存在——不单票、不计票**（cookieMissing 对未登录上下文近乎恒真，保留凑票会让 skip 记忆形同虚设）；裁决翻转靠删除该条目（经其他信号触发的 viewer 完成登录不会改写 skip 记录）。viewer 打开时工具栏显示触发原因。人工登录走 CDP Screencast 中继（`screencast.mjs`：无头 chromium → HTTP+WS viewer，JS/CSS 全内联；**WS 消息监听必须在任何 await 之前挂载**——客户端可能在握手完成后立即发消息，EventEmitter 不排队无监听期的帧）。viewer 起来后**自动 `open` 用户默认浏览器**（原 login_url.mjs 行为，snapshot 合并时遗失、已恢复；`U2M_VIEWER_NOOPEN=1` 关闭，测试用）；登录期间页面视口临时切 1280×800（与 viewer 画布匹配——共享上下文是 1280×3000 懒加载视口，3000px 塞 800px 画布会纵向压扁），结束后恢复。viewer 地址以 `[snapshot] viewer: http://...` 记录到 stderr，测试靠它接入。
