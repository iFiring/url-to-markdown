...
**snapshot.mjs 单入口 + lib/ 模块**。步骤 1（快照下载 + 结构清洗，2026-09-11 起原步骤 1/2 合并）是单个 CLI `script/snapshot.mjs`，内部按阶段调用六个阶段模块：`snapshot-login.mjs`（登录检测 + Screencast viewer）、`snapshot-scroll.mjs`（渐进滚动 + DOM 稳定）、`snapshot-redirect.mjs`（占优内容 iframe 重定向门）、`snapshot-detect.mjs`（虚拟列表检测门）、`snapshot-capture.mjs`（全保真快照抓取）、`clean-snapshot.mjs`（两趟结构清洗，原步骤 2 CLI 主体平移）。单个 chromium 实例贯穿全流程——五阶段抓取 context 带 storageState+page-init、用毕即关；清洗阶段开裸 context（无 storageState/initScripts + route-abort http(s)，环境隔离是设计约束：file:// 重解析只应用内联后的 `<style>`，computed style 须来自纯净级联）。各模块不直接 emit——它们抛异常或返回值，由 snapshot.mjs 统一处理 emit 逻辑（清洗模块 finally 只关自己的 context，browser 归 CLI 关）。
...
- **步骤 1 `snapshot.mjs --url` —— 合并七阶段（五阶段抓取 + 清洗）+ 核心参数产出**
  - 登录检测 `snapshot-login.mjs` → 渐进滚动 `snapshot-scroll.mjs` → **重定向门** `snapshot-redirect.mjs` → 虚拟列表检测 `snapshot-detect.mjs`（跑在最终目标页上）→ 全保真快照 `snapshot-capture.mjs` → **清洗阶段** `clean-snapshot.mjs`（两趟渲染 + Node 层表格/代码块转换，详述见下方「步骤 1 清洗阶段」节）
